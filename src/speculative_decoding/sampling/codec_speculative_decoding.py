...
from speculative_decoding.utils.caching import prune_cache
...
def speculative_generate_encoder_decoder(
    ...
    prompt_drafter = torch.tensor(
        inputs, dtype=torch.long, device=drafter.device
    ).unsqueeze(0)
    max_seq_length = (
    ...
    while current_position < total_len:
        ...
        stop_locations = torch.nonzero(
            torch.eq(
                decoded_input_ids[..., current_position : current_position + n],
                stop_tokens,
            )
        )
        if stop_locations.shape[0] > 0:
            ...
            return decoded_input_ids[
                0, : current_position + stop_location + 1
